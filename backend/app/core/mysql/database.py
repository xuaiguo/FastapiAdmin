...
懒加载创建 MySQL 异步引擎和会话工厂，按 config_id 缓存。
连接配置从 MySQL 的 sys_mysql_config 表中读取。
"""
...
class MySQLManager:
    """
    MySQL 多数据源数据库引擎管理器（单例）。

    职责:
    - 根据 config_id 懒加载创建 MySQL 异步引擎
    - 缓存引擎和会话工厂，避免重复创建
    - 应用关闭时统一释放所有连接池
    """
    ...
    async def _load_config(self, config_id: int) -> dict:
        """从 MySQL 读取 MySQL 连接配置"""
        from app.api.v1.module_system.mysql_config.model import MysqlConfigModel
        from app.core.database import async_db_session
        from app.core.mysql.crypto import decrypt_password

        async with async_db_session() as session:
            stmt = select(MysqlConfigModel).where(
                MysqlConfigModel.id == config_id,
                MysqlConfigModel.is_deleted == False,  # noqa: E712
                MysqlConfigModel.status == 0,
            )
            result = await session.execute(stmt)
            config = result.scalars().first()
            if not config:
                raise ValueError(f"MySQL 配置不存在或已禁用: config_id={config_id}")
            return {
                "name": config.name,
                "host": config.host,
                "port": config.port,
    ...
    def _build_url(self, config: dict) -> str:
        ...
        database_name = config["database_name"]
        charset = config["charset"]
        return f"mysql+aiomysql://{username}:{password}@{host}:{port}/{database_name}?charset={charset}"

    async def get_engine(self, config_id: int) -> AsyncEngine:
        """获取 MySQL 异步引擎（懒加载 + 缓存）"""
    ...
    async def dispose_all(self) -> None:
        """释放所有 MySQL 连接池（应用关闭时调用）"""
        for config_id, engine in self._engines.items():
            await engine.dispose()
            logger.info("✅ MySQL 引擎已释放: config_id={}", config_id)
        self._engines.clear()
    ...
    async def invalidate_engine(self, config_id: int) -> None:
        ...
        async with self._lock:
            if config_id in self._engines:
                ...
                logger.info("🔄 MySQL 引擎缓存已失效: config_id={}", config_id)
